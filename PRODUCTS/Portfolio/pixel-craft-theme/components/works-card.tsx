...
export function WorksCard({ title, description, slug, imageUrl, tags, index = 0 }: WorksCardProps) {
  return (
    <div
      ...
      className="pixel-card overflow-hidden h-full flex flex-col pixel-card-animate transition-all duration-200 hover:scale-[1.02]"
    >
      <div className="relative h-56 group overflow-hidden">
        ...
        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-center justify-center">
          <Link 
            href={`/works/${slug}`}
            className="bg-primary text-black font-bold px-6 py-3 border-2 border-black flex items-center transition-transform duration-200 translate-y-4 group-hover:translate-y-0"
          >
